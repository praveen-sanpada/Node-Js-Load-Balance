...
redisSub.subscribe('client-server', (err, count) => {
    if (err) {
        ...
        console.log(`Server1 subscribed to ${count} channel(s)`);
    }
});

redisSub.on('message', (channel, message) => {
    console.log("I am in Server1.")
    if (channel === 'client-server') {
        const quiz = JSON.parse(message);
        // console.log(quiz)
        console.log(`Server1 received message from MainServer(current_date): ${quiz.quiz.current_date}`);
    }
});
