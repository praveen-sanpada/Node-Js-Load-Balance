...
redisSub.subscribe('client-server', (err, count) => {
    if (err) {
        ...
        console.log(`Server5 subscribed to ${count} channel(s)`);
    }
});

redisSub.on('message', (channel, message) => {
    console.log("I am in Server5.")
    if (channel === 'client-server') {
        const quiz = JSON.parse(message);
        // console.log(quiz)
        console.log(`Server5 received message from MainServer(current_date): ${quiz.quiz.current_date}`);
    }
});

const PORT = 3005;
server.listen(PORT, () => {
    console.log(`Server5 is running on port ${PORT}`);
});
